...
public protocol ForkResultFourthOperationProtocol {
    associatedtype Success
    associatedtype Failure: Error
    associatedtype PreviousOperation: ForkResultThirdOperationProtocol
...
public final class ForkResultFourthOperation<S, E, X>: AsyncOperation, ForkResultFourthOperationProtocol
where E: Error, X: ForkResultThirdOperationProtocol & Operation {
...
public extension ForkResultFourthOperation {
    ...
    func join<NewSuccess, NewFailure: Error>(
        ...
        let operation = ResultOperation<NewSuccess, Error>(operationQueue: operationQueue) { [unowned self] resolve in
            do {
                let (value1, value2, value3, value4) = try ReadonlyResultMapper.map(
                ...
                makeOperation(value1, value2, value3, value4) { result in
    ...
    ///                      you need to return the resulting value to the function
    /// - Returns: a fork result FOURTH operation
    ///
    ...
    func fork<NewSuccess, NewFailure: Error>(
        ...
        _ failureType: NewFailure.Type = NewFailure.self,
        makeOperation: @escaping (
            PreviousOperation.PreviousOperation.PreviousOperation.PreviousOperation.Success,
            @escaping (Result<NewSuccess, NewFailure>) -> Void
        ) -> Void
    ) -> ForkResultFifthOperation<NewSuccess, Error , ForkResultFourthOperation> {
        let operation = ForkResultFifthOperation<NewSuccess, Error, ForkResultFourthOperation>(
            operationQueue: operationQueue,
            previousOperation: self,
            operationBlock: { [unowned self] resolve in
                do {
                    let value = try self.previousOperation.previousOperation.previousOperation.previousOperation.result.get()!.get()
    ...
    func onCompletion(
         ...
         completion: @escaping (PreviousPreviousPreviousResult, PreviousPreviousResult, PreviousResult, CurrentResult) -> Void
     ) {
         let operation = AsyncBlockOperation { [unowned self] completionAsyncBlock in
             if let result1 = self.previousOperation.previousOperation.previousOperation.result.get(),
                let result2 = self.previousOperation.previousOperation.result.get(),
                let result3 = self.previousOperation.result.get(),
                let result4 = self.result.get() {
